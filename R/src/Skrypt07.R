...
data8 <- readr::read_csv(file = "Github"
                         # Wczytaj dane z pliku excel na dysku, wyraźnie wskaż arkusz
                         data9 <- readxl::read_excel("Github", sheet = 1)
                         # {dplyr} i {tidyr}
                         ...
                         dane23 <- pingwiny |> 
                           tidyr::pivot_longer(cols = c(bill_length_mm, bill_depth_mm), names_to = "nazwa.zmiennej", values_to = "wartosc_zmiennej")
                         ...
                         data25 <- read_xlsx(path = "/GitHub/przetwarzanie_danych_projekt_semestralny/R/data/data_msu.xlsx")
                         data26 <- left_join(ramk1,ramk2, join_by(sample.id == sample.id))
                         ...
                         data27 <- right_join(ramk2, ramk3, join_by(sample.id))
                         # Korzystając z danych geochemicznych połącz ze sobą wszystkie trzy ramki w sposób całkowity
                         data28 <- full_join(ramk1, ramk2, ramk3, join_by(sample.id)) |> full_join((x = _, y = ramk2, join_by(sample.id))